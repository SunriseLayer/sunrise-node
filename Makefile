...
fmt: sort-imports
	...
	@find . -name '*.go' -type f -not -path "*.git*"  -not -name '*.pb.go' -not -name '*pb_test.go' | xargs goimports -w -local github.com/sunirse-zone
...
PB_SUNRISE_APP=$(shell go list -f {{.Dir}} -m github.com/sunriselayer/sunrise)
